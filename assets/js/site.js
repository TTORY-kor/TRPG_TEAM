...
function imageUrl(value, fallback = '') {
  ...
  return IMG + path.replace(/^assets\/images\//, '');
}
...
function applySite(site) {
  ...
  $('#heroImage').src = imageUrl(site.heroImage, 'office.jpg');
  $('#profileImage').src = imageUrl(site.profileImage, 'icon.jpg');
  ...
  $('#mainImage').src = imageUrl(site.mainImage, 'main.png');
...
function renderCharacters(list) {
  $('#characterShortcuts').innerHTML = list.map((character) =>
    `<button style="background-image:url('${imageUrl(character.banner)}')" data-char="${esc(character.id)}"><span>${esc(character.name)}</span></button>`
  ).join('');
  $('#characterList').innerHTML = list.map((character) => `
    <article class="character-card" id="char-${esc(character.id)}" style="--char-accent:${esc(character.accent)}">
      <div class="character-banner" style="background-image:url('${imageUrl(character.banner)}')"><h3>${esc(character.name)}</h3></div>
      <div class="character-body">
        <img class="standing" src="${imageUrl(character.standing)}" alt="${esc(character.name)}">
        <div>
...
function renderNpc(list) {
  ...
  $('#npcGrid').innerHTML = list.map((npc) => `<article class="npc-card"><span class="badge" style="background:${colors[npc.relation] || '#444'}">${esc(npc.relation)}</span><img src="${imageUrl(npc.image)}" alt="${esc(npc.name)}"><h3>${esc(npc.name)}</h3><p>${esc(npc.description)}</p></article>`).join('');
}
function renderGallery(list) {
  $('#galleryGrid').innerHTML = list.map((item,index) => `<button data-i="${index}"><img src="${imageUrl(item.image)}" alt="gallery ${index + 1}"></button>`).join('');
  $('#galleryGrid').onclick = (event) => {
    const button = event.target.closest('button'); if (!button) return;
    const item = list[Number(button.dataset.i)];
    $('#lightboxImage').src = imageUrl(item.image); $('#lightboxCredit').textContent = item.credit || ''; $('#lightbox').showModal();
  };
}
